...
public class GameMap implements IGameMap {
    ...
    public List<ILocation> getNeighbourhood(ILocation loc, int dist) {
        ...
        ArrayList<ILocation> neighbours = new ArrayList<>();
        if (dist < 0 || loc == null)
        ...
        int xStart = loc.getX();
        int yStart = loc.getY();
        for (int distance = 1; distance <= dist; distance++) {
            int currentX = xStart;
            int currentY = yStart;
            ...
            if (currentX <= getWidth() && currentX >= 0 && currentY <= getHeight() && currentY >= 0) {
                ILocation lcn = getLocation(currentX, currentY);
                if (!hasWall(lcn)) {
                    neighbours.add(lcn);
                }
            }
            for (int i = 1; i <= distance * 2; i++) {
                currentX++;
                if (currentX <= getWidth() && currentX >= 0 && currentY <= getWidth() && currentY >= 0) {
                    ILocation lcn = getLocation(currentX, currentY);
                    if (!hasWall(lcn)) {
                        neighbours.add(lcn);
                    }
